...
public class CookieUtils {
    ...
    private static final String AGE_COOKIE_NAME = "age"; // 사용자 나이 쿠키 이름
    private static final String GENDER_COOKIE_NAME = "gender"; // 사용자 성별 쿠키 이름
    ...
    public static void deleteAuthCookies(HttpServletRequest request, HttpServletResponse response) {
        ...
        deleteCookie(request, response, USER_ACTIVITY_INFO_COOKIE_NAME);
        deleteCookie(request, response, AGE_COOKIE_NAME);
        deleteCookie(request, response, GENDER_COOKIE_NAME);
    }
     ...
     * 사용자 나이를 age 쿠키로 저장
     */
    public static void addAgeCookie(HttpServletResponse response, Integer age) {
        if (age != null) {
            ResponseCookie ageCookie = ResponseCookie.from(AGE_COOKIE_NAME, age.toString())
                    .path("/")
                    .maxAge(DEFAULT_MAX_AGE)
                    ...
                    .sameSite("None")
                    .build();
            response.addHeader(HttpHeaders.SET_COOKIE, ageCookie.toString());
        }
    }

    /**
     * 사용자 성별을 gender 쿠키로 저장
     */
    public static void addGenderCookie(HttpServletResponse response, Integer gender) {
        if (gender != null) {
            ResponseCookie genderCookie = ResponseCookie.from(GENDER_COOKIE_NAME, gender.toString())
                    .path("/")
                    .maxAge(DEFAULT_MAX_AGE)
                    .httpOnly(false) // JavaScript에서 접근 가능하도록 설정
                    .secure(true) // HTTPS에서만 전송되도록 설정
                    .sameSite("None")
                    .build();
            response.addHeader(HttpHeaders.SET_COOKIE, genderCookie.toString());
        }
    }

    /**
     * 사용자 정보 한 번에 설정 - 쿠키 모두 설정 (편의 메서드)
     */
    public static void addAllUserCookies(HttpServletResponse response, UserDto userDto, String refreshToken) {
        ...
        // 5. 개별 age, gender 쿠키 추가
        addAgeCookie(response, userDto.getAge());
        addGenderCookie(response, userDto.getGender());
    }
     ...
     * 요청에서 나이 쿠키 읽기
     */
    public static Optional<Integer> getAgeFromCookie(HttpServletRequest request) {
        Optional<Cookie> cookieOpt = getCookie(request, AGE_COOKIE_NAME);
        return cookieOpt.map(cookie -> {
            try {
     ...
     * 요청에서 성별 쿠키 읽기
     */
    public static Optional<Integer> getGenderFromCookie(HttpServletRequest request) {
        Optional<Cookie> cookieOpt = getCookie(request, GENDER_COOKIE_NAME);
        return cookieOpt.map(cookie -> {
            try {
                return Integer.parseInt(cookie.getValue());
            } catch (NumberFormatException e) {
