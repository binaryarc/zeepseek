...
const ZzimList = () => {
  ...
  const drawOverlayAndMarkers = async (room) => {
    ...
    if (window.updateCurrentLocationLabel) {
  ...
  const handleRoomClick = (room) => {
    dispatch(
      setSelectedPropertyId(
        selectedPropertyId === room.propertyId ? null : room.propertyId
      )
    );
  ...
  const toggleLike = async (room) => {
    ...
    try {
      ...
      setRooms((prevRooms) =>
        prevRooms.filter((r) => r.propertyId !== propertyId)
      );
